        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div
                key={idx}
                className={`p-4 rounded-xl shadow-md bg-gradient-to-tr from-slate-900 to-slate-800 border ${
                  metric.highlight ? 'border-red-500/50' : 'border-slate-700'
                }`}
              >
                <div className="text-xs text-slate-400">{metric.label}</div>
                <div className={`text-lg font-semibold ${metric.highlight ? 'text-red-300' : 'text-cyan-100'}`}>
                  {String(metric.value)}
                </div>
              </div>
            ))}
