...
const SigmaPage = () => {
  ...
  const fetchAll = async () => {
    try {
      const [statusRes, coverageRes, rulesRes] = await Promise.all([
        apiClient.get(`/sigma/status`),
        apiClient.get(`/sigma/coverage`),
        apiClient.get(`/sigma/rules?limit=25`),
      ]);
  ...
  const searchRules = async () => {
    try {
      const res = await apiClient.get(`/sigma/rules?limit=50&query=${encodeURIComponent(ruleQuery)}`, { headers });
  ...
  const reloadRules = async () => {
    setReloading(true);
    try {
      const res = await apiClient.post(`/sigma/reload`, {});
