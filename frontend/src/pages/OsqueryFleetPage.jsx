...
const OsqueryFleetPage = () => {
  ...
  const loadOverview = async () => {
    try {
      const [statusRes, statsRes, hostsRes, queriesRes, resultsRes] = await Promise.all([
        axios.get(`${API}/osquery/status`, { headers }),
        axios.get(`${API}/osquery/stats`, { headers }),
        axios.get(`${API}/osquery/hosts?limit=25`, { headers }),
        axios.get(`${API}/osquery/queries?limit=30`, { headers }),
        axios.get(`${API}/osquery/results?limit=30`, { headers }),
      ]);
  ...
  const searchQueries = async () => {
    try {
      const res = await axios.get(`${API}/osquery/queries?limit=50&query=${encodeURIComponent(querySearch)}`, { headers });
