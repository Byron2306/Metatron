...
const ZeekPage = () => {
  ...
  const fetchOverview = async () => {
    try {
      const [statusRes, statsRes, typesRes] = await Promise.all([
        apiClient.get(`/zeek/status`),
        apiClient.get(`/zeek/stats`),
        apiClient.get(`/zeek/log-types`),
      ]);
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const res = await apiClient.get(`/zeek/logs/${selectedLog}?limit=${lim}`);
  ...
  const fetchDetections = async () => {
    try {
      const [beaconRes, dnsRes] = await Promise.all([
        apiClient.get(`/zeek/detections/beaconing?limit=20`),
        apiClient.get(`/zeek/detections/dns-tunneling?limit=20`),
      ]);
