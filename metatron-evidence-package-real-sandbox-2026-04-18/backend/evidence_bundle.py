...
def run_has_real_execution(run: Dict[str, Any]) -> bool:
    """True only when the atomic test actually ran.

    This supports both sandboxed local executions and remote Windows
    executions that report real_execution as the outcome.
    """
    ...
    if (
        ...
        and int(exit_code) == 0
        and str(run.get("outcome") or "").strip().lower() == "real_execution"
    ):
        return True
    return (
        run.get("status") == "success"
...
def run_is_real_sandbox_execution(run: Dict[str, Any]) -> bool:
    if not run_has_real_execution(run):
        return False
    if str(run.get("outcome") or "").strip().lower() == "real_execution":
        return True
    sandbox_marker = run.get("sandbox")
    if sandbox_marker == "docker-network-none-cap-drop-all":
        return True
    if str(run.get("execution_mode") or "").strip().lower() != "sandbox":
        return False
    command_repr = str(run.get("command") or run.get("command_line") or "")
    return "--network', 'none" in command_repr or "--network none" in command_repr
