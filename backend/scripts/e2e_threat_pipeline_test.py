...
class ThreatPipelineE2E:
    ...
    def _poll_until(self, path: str, check_key: str = "status", success_values=None, timeout_s: int = 60) -> Optional[Dict[str, Any]]:
        success_values = success_values or ["completed", "finished", "done", "available"]
        deadline = time.time() + timeout_s
        while time.time() < deadline:
            try:
                resp = self._request(f"poll_{path}", "GET", path, expected_codes=[200, 202], domain="polling")
                data = resp.json() or {}
                val = data.get(check_key)
                if isinstance(val, str) and val.lower() in success_values:
                    return data
            except Exception:
                pass
            time.sleep(2)
    ...
    def _simulate_sandbox_trivy_and_integration_checks(self) -> None:
        # Cuckoo / sandbox submit + poll for report
        submit = self._request(
            "sandbox_submit",
            "POST",
            "/advanced/sandbox/submit/url",
            json_body={"url": "http://malicious.example/payload.exe"},
            expected_codes=[200, 201, 202],
            domain="sandbox",
        )
        task_info = submit.json() or {}
        # Advanced/gated submissions return queue/decision ids; actual sandbox task
        # id may not be present until approval. Only poll if a task id is returned.
        task_id = task_info.get("task_id") or task_info.get("id")
        if task_id:
            report = self._poll_until(f"/advanced/sandbox/report/{task_id}", check_key="status", timeout_s=90)
            self.artifacts["sandbox_report_available"] = bool(report)

        # Container image deep scan via Trivy -> poll for result
        scan = self._request(
            "container_image_scan_deep",
            "POST",
            "/containers/scan",
            json_body={"image_name": "nginx:latest", "force": True},
            expected_codes=[200, 202, 201],
            domain="container_security",
        )
        scan_info = scan.json() or {}
        # Router stores scan results keyed by image_name and exposes a history endpoint.
        image_name = (scan_info.get("image_name") or scan_info.get("image") or "nginx:latest")
        if image_name:
            # Poll scan history and look for an entry matching the image name
            deadline = time.time() + 90
            found = None
            while time.time() < deadline:
                try:
                    resp = self._request("poll_containers_scans_history", "GET", "/containers/scans/history", expected_codes=[200], domain="container_security")
                    payload = resp.json() or {}
                    scans = payload.get("scans") or []
                    for s in scans:
                        if s.get("image_name") == image_name or s.get("image") == image_name:
                            found = s
                            break
                    if found:
                        break
                    ...
                    pass
                time.sleep(2)
            self.artifacts["container_scan_report_available"] = bool(found)

        # Ollama / local LLM ping via integration endpoint
        self._request(
            "ollama_ping",
            "GET",
            "/integrations/ollama/ping",
            expected_codes=[200, 204],
            domain="integrations",
        )

        # Generic integrations discovery
        self._request(
            "integrations_list",
            "GET",
            "/integrations",
    ...
    def _simulate_celery_task_roundtrip(self) -> None:
        # Trigger a background task that should enqueue via Celery and then poll status
        trigger = self._request(
            "celery_trigger_sample_task",
            "POST",
            "/tasks/trigger-sample",
            json_body={"task_name": "e2e_sample", "payload": {"x": 1}},
            expected_codes=[200, 202],
            domain="celery",
        )
        info = trigger.json() or {}
        task_id = info.get("task_id") or info.get("id")
        if task_id:
            task_report = self._poll_until(f"/tasks/{task_id}", check_key="state", success_values=["success", "finished"], timeout_s=60)
            self.artifacts["celery_task_completed"] = bool(task_report)
    ...
    def run(self) -> Dict[str, Any]:
        ...
        # Additional sandbox/trivy/integration checks and Celery roundtrip
        try:
            self._simulate_sandbox_trivy_and_integration_checks()
        except Exception:
            pass
        try:
            self._simulate_celery_task_roundtrip()
        except Exception:
            pass
