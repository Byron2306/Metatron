...
Maze state is per session/campaign and persists in memory with optional MongoDB durability.
"""
...
_FAKE_FILE_PATHS = [
    "/synthetic/vault/config.hcl", "/synthetic/users/svc_deploy/ssh_bundle.txt",
    "/synthetic/app/config/database.yml", "/synthetic/kubernetes/admin_bundle.yaml",
    "/synthetic/jenkins/aws_credentials.txt", "/synthetic/root/kube_config.yaml",
    "/synthetic/terraform/terraform.lock.hcl", "/synthetic/consul/consul.hcl",
]
...
def _make_fake_credential(rng: random.Random, username: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        "username": uname,
        "password": password,
        "api_token": f"syn-{token_hex[:20]}",
        "aws_access_key_id": f"SYN{''.join(rng.choices('ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', k=17))}",
...
def _make_fake_host(rng: random.Random) -> Dict[str, Any]:
    ...
    return {
        ...
        "domain": f"{hostname}.synthetic.invalid",
...
class MystiqueMaze:
    ...
    def _wrap_payload_with_context(
        ...
        return {
            ...
            "_seraph_synthetic": True,
            "_seraph_synthetic_class": "mirror_world",
            "_seraph_synthetic_origin": "mystique_maze",
        }
