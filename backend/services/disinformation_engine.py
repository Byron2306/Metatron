...
def _gen_api_schema(rng: random.Random) -> Dict[str, Any]:
    ...
    return {
        "schema_family": "synthetic-api-map",
        "schema_version": f"2.{rng.randint(1,9)}.0",
        "catalog": {
            "users_index": {
                "method": "GET",
                "summary": "List users",
            ...
            "vault_lookup": {
                ...
                "parameters": [
                    {"name": "session_token", "in": "header", "required": True},
                    {"name": "namespace", "in": "query", "default": "synthetic"},
                ],
            },
            "admin_export": {
                "method": "POST",
...
def _gen_identity(rng: random.Random) -> Dict[str, Any]:
    ...
    for uname in rng.sample(_FAKE_USERS, k=rng.randint(4, 8)):
        users.append({
            "username": uname,
            "email": f"{uname}@synthetic.invalid",
    ...
    return {"users": users, "total": len(users), "directory": "synthetic-ldap"}
...
class DisinformationEngine:
    ...
    async def persist_serve(
        ...
        *,
        ...
        if self._db is None or not hasattr(self._db, "disinformation_serves"):
            return False
        try:
            await self._db.disinformation_serves.replace_one(
            ...
            logger.exception("DISINFORMATION: failed to persist serve %s", record.serve_id)
            return False
    ...
    def generate(
    ...
    ) -> tuple[Dict[str, Any], DisinfoServe]:
        """
        ...
        response = {
            ...
            "_seraph_synthetic_class": "disinformation",
            "_seraph_synthetic_origin": "disinformation_engine",
        }
