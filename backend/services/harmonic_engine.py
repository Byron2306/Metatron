...
class HarmonicEngine:
    ...
    def score_observation(
        ...
        *,
        ...
        resonance = self.compute_resonance_score(features, baseline_ref)
        discord = self.compute_discord_score(features, baseline_ref)
        confidence = self.compute_confidence(
            features,
            sample_size=features.sample_size,
            env_conditions={
                "baseline_quality": 0.85 if baseline_ref.scope_type != "global_fallback" else 0.45,
                "environment_state": "degraded"
                if str(environment or "").lower() in {"incident", "degraded"}
                else "normal",
            },
        )
        mode_recommendation, rationale = self._mode_recommendation(resonance, discord, confidence)
