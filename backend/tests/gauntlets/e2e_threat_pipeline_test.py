...
def _assert_e2e_server_available() -> None:
    try:
        requests.get(BASE_URL, timeout=2)
    except (requests.RequestException, OSError, PermissionError) as exc:
        pytest.skip(f"E2E threat pipeline requires reachable server at {BASE_URL}: {exc}")


def test_threat_pipeline():
    """Synchronous wrapper for pytest compatibility."""
    _assert_e2e_server_available()
    main()
