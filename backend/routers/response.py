...
async def analyze_with_openclaw(threat_data: dict, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # entry points accept a canonical snapshot. This keeps behavior consistent
        # between triune services and optional OpenClaw enrichment.
        ctx = ReasoningContext(
            entities=[],
            relationships={},
            evidence_set=[threat_data],
            trust_state={},
            timeline_window=[{"ts": datetime.now(timezone.utc).isoformat(), "event": "reported_threat", "data": threat_data}],
            window_seconds=3600,
        )
        ...
        try:
            if hasattr(openclaw, "analyze_threat"):
                # openclaw expects a ThreatContext; we keep best-effort behavior
                tc = ThreatContext(
                ...
                openclaw_result = await openclaw.analyze_threat(tc)
        except Exception:
            openclaw_result = {"error": "openclaw_unavailable"}

        return {"snapshot_analysis": analysis, "openclaw": openclaw_result}
    except Exception as e:
