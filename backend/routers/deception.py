...
async def record_decoy_interaction(request: DecoyInteractionRequest):
    ...
    # ingest interaction into world model
    from routers.dependencies import get_db
    from services.world_model import WorldModelService, WorldEntity, WorldEdge
    db = get_db()
    wm = WorldModelService(db)
    # ensure entity for IP exists
    await wm.upsert_entity(WorldEntity(id=request.ip, type="agent", attributes={"last_seen": assessment.score}))
    # ensure entity for decoy exists
    # use "alert" type for decoy entities (enum constraint)
