...
async def receive_scanner_report(
    ...
    # Only perform auto-deploy if the server is explicitly configured to allow it.
    allow_auto_env = os.environ.get("ALLOW_SCANNER_AUTO_DEPLOY", "false").lower() in ("1", "true", "yes")
    if request.auto_deploy_request and allow_auto_env:
        from services.agent_deployment import get_deployment_service, start_deployment_service

        deployment_service = get_deployment_service()
    elif request.auto_deploy_request and not allow_auto_env:
        logger.info("Scanner requested auto-deploy but server ALLOW_SCANNER_AUTO_DEPLOY is not enabled; skipping deployment queueing")
        deployment_service = None
