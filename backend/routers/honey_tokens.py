...
async def create_honey_token(
    ...
    try:
        ...
        await emit_world_event(get_db(), event_type="honey_token_created", entity_refs=[token["id"]], payload={"token_type": request.token_type, "location": request.location}, trigger_triune=False)
        return token
...
async def delete_honey_token(
    ...
    await emit_world_event(get_db(), event_type="honey_token_deleted", entity_refs=[token_id], payload={"actor": current_user.get("id")}, trigger_triune=False)
...
async def toggle_honey_token(
    ...
    if not token:
        raise HTTPException(status_code=404, detail="Token not found")
    await emit_world_event(get_db(), event_type="honey_token_toggled", entity_refs=[token_id], payload={"is_active": token["is_active"], "actor": current_user.get("id")}, trigger_triune=False)
...
async def check_honey_token(
    ...
    if token:
        ...
        await emit_world_event(
            get_db(),
            event_type="honey_token_checked",
            entity_refs=[token.id],
            payload={"matched": True, "source_ip": source_ip, "campaign_tracking": campaign_info},
    ...
    await emit_world_event(get_db(), event_type="honey_token_checked", entity_refs=[], payload={"matched": False}, trigger_triune=False)
