#!/usr/bin/env python3
"""
run_arda_prevention_full_v5.py
===============================
Phase 1 Full v5: Expanded 648-technique enforcement sweep.

Includes all 516 techniques from v3 PLUS 132 additional techniques covering:
  - Missing base techniques (T1003, T1016, T1021, T1027, T1040, T1041, etc.)
  - Missing sub-techniques (T1059.001, T1059.004, T1071.003, T1071.004, etc.)
  - Full Reconnaissance tactic coverage (T1589–T1598)
  - Additional credential/collection/impact techniques

Pre-writes 648 payloads BEFORE enforcement.
Starts Arda enforcement (180s window with 30s delay).
Waits 65s for enforcement to activate.
Executes 648 payloads via os.fork+execve while enforcement is ACTIVE.
Stops container immediately after.
"""
import os
import json
import subprocess
import sys
import time
from pathlib import Path
from datetime import datetime, timezone

REPO = Path(__file__).resolve().parent.parent

# ============================================================================
# 648-technique master list
# v3 original 516 + 132 additions to reach 648 total
# ============================================================================
TECHNIQUES = [
    # --- C2 ---
    "T1001", "T1001.001", "T1001.002", "T1001.003",
    # --- Credential Access ---
    "T1003", "T1003.001", "T1003.002", "T1003.003",
    "T1003.004", "T1003.005", "T1003.006", "T1003.007", "T1003.008",
    "T1006", "T1008", "T1010",
    "T1011", "T1011.001", "T1014",
    "T1016", "T1016.001", "T1016.002",
    "T1020.001",
    "T1021", "T1021.001", "T1021.002", "T1021.003", "T1021.004",
    "T1021.005", "T1021.006", "T1021.007", "T1021.008",
    "T1027", "T1027.001", "T1027.002", "T1027.003", "T1027.004", "T1027.005",
    "T1027.006", "T1027.007", "T1027.008", "T1027.009", "T1027.010", "T1027.011",
    "T1027.012", "T1027.013", "T1027.014", "T1027.015", "T1027.016", "T1027.017",
    "T1036", "T1036.001", "T1036.002", "T1036.003", "T1036.004", "T1036.005",
    "T1036.006", "T1036.007", "T1036.008", "T1036.009", "T1036.010", "T1036.011",
    "T1036.012",
    "T1037", "T1037.001", "T1037.002", "T1037.003", "T1037.004", "T1037.005",
    "T1040", "T1041", "T1046",
    "T1047",
    "T1048", "T1048.001", "T1048.002", "T1048.003",
    "T1049",
    "T1052", "T1052.001",
    "T1053", "T1053.002", "T1053.003", "T1053.005", "T1053.006", "T1053.007",
    "T1055", "T1055.001", "T1055.002", "T1055.003", "T1055.004", "T1055.005",
    "T1055.008", "T1055.009", "T1055.011", "T1055.012", "T1055.013", "T1055.014",
    "T1055.015",
    "T1056", "T1056.001", "T1056.002", "T1056.003", "T1056.004",
    "T1057",
    "T1059", "T1059.001", "T1059.002", "T1059.003", "T1059.004", "T1059.005",
    "T1059.006", "T1059.007", "T1059.008", "T1059.009", "T1059.010", "T1059.011",
    "T1059.012", "T1059.013",
    "T1069", "T1069.001", "T1069.002", "T1069.003",
    "T1070", "T1070.001", "T1070.002", "T1070.003", "T1070.004", "T1070.005",
    "T1070.006", "T1070.007", "T1070.008", "T1070.009", "T1070.010",
    "T1071", "T1071.001", "T1071.002", "T1071.003", "T1071.004", "T1071.005",
    "T1072",
    "T1074", "T1074.001", "T1074.002",
    "T1078", "T1078.001", "T1078.002", "T1078.003", "T1078.004",
    "T1080",
    "T1082", "T1083",
    "T1087", "T1087.001", "T1087.002", "T1087.003",
    "T1090", "T1090.001", "T1090.002", "T1090.003", "T1090.004",
    "T1092", "T1095",
    "T1098", "T1098.001", "T1098.002", "T1098.003", "T1098.004",
    "T1098.005", "T1098.006", "T1098.007",
    "T1102", "T1102.001", "T1102.002", "T1102.003",
    "T1104", "T1105", "T1106",
    "T1110.001", "T1110.002", "T1110.003", "T1110.004",
    "T1111",
    "T1113",
    "T1114", "T1114.001", "T1114.002", "T1114.003",
    "T1119",
    "T1120", "T1123", "T1124", "T1125",
    "T1127", "T1127.001", "T1127.002", "T1127.003",
    "T1129",
    "T1132.001", "T1132.002",
    "T1133",
    "T1134", "T1134.001", "T1134.002", "T1134.003", "T1134.004", "T1134.005",
    "T1135",
    "T1136", "T1136.001", "T1136.002",
    "T1137", "T1137.001", "T1137.002", "T1137.003", "T1137.004", "T1137.005",
    "T1137.006",
    "T1140",
    "T1176", "T1176.001", "T1176.002",
    "T1185", "T1187", "T1189", "T1190",
    "T1195", "T1195.001", "T1195.002", "T1195.003",
    "T1197", "T1199", "T1200",
    "T1202", "T1203",
    "T1204", "T1204.001", "T1204.002", "T1204.003", "T1204.004", "T1204.005",
    "T1205", "T1205.001", "T1205.002",
    "T1207", "T1210", "T1211", "T1212",
    "T1213", "T1213.001", "T1213.002", "T1213.003", "T1213.004", "T1213.005",
    "T1213.006",
    "T1216", "T1216.001", "T1216.002",
    "T1217",
    "T1218", "T1218.001", "T1218.002", "T1218.003", "T1218.004", "T1218.005",
    "T1218.007", "T1218.008", "T1218.009", "T1218.010", "T1218.011", "T1218.012",
    "T1218.013", "T1218.014", "T1218.015",
    "T1219", "T1219.001", "T1219.002", "T1219.003",
    "T1220", "T1221",
    "T1222.001", "T1222.002",
    "T1480", "T1480.001", "T1480.002",
    "T1484", "T1484.001", "T1484.002",
    "T1485", "T1485.001",
    "T1486", "T1489", "T1490",
    "T1491", "T1491.001", "T1491.002",
    "T1495",
    "T1496.001", "T1496.002", "T1496.003", "T1496.004",
    "T1497.001", "T1497.002", "T1497.003",
    "T1498", "T1498.001", "T1498.002",
    "T1499", "T1499.001", "T1499.002", "T1499.003", "T1499.004",
    "T1505", "T1505.001", "T1505.002", "T1505.003", "T1505.004", "T1505.005",
    "T1505.006",
    "T1518", "T1518.001", "T1518.002",
    "T1525", "T1526", "T1528", "T1529", "T1530", "T1531", "T1534", "T1535",
    "T1537", "T1538", "T1539",
    "T1542", "T1542.001", "T1542.002", "T1542.003", "T1542.004", "T1542.005",
    "T1543", "T1543.001", "T1543.002", "T1543.003", "T1543.004", "T1543.005",
    "T1546", "T1546.001", "T1546.002", "T1546.003", "T1546.004", "T1546.005",
    "T1546.006", "T1546.007", "T1546.008", "T1546.009", "T1546.010", "T1546.011",
    "T1546.012", "T1546.013", "T1546.014", "T1546.015", "T1546.016", "T1546.017",
    "T1546.018",
    "T1547", "T1547.001", "T1547.002", "T1547.003", "T1547.004", "T1547.005",
    "T1547.006", "T1547.007", "T1547.008", "T1547.009", "T1547.010", "T1547.011",
    "T1547.012", "T1547.013", "T1547.014", "T1547.015",
    "T1548", "T1548.001", "T1548.002", "T1548.003", "T1548.004", "T1548.005",
    "T1548.006",
    "T1550", "T1550.001", "T1550.002", "T1550.003", "T1550.004",
    "T1552", "T1552.001", "T1552.002", "T1552.003", "T1552.004", "T1552.005",
    "T1552.006", "T1552.007", "T1552.008",
    "T1553", "T1553.001", "T1553.002", "T1553.003", "T1553.004", "T1553.005",
    "T1553.006",
    "T1555", "T1555.001", "T1555.002", "T1555.003", "T1555.004", "T1555.005",
    "T1555.006",
    "T1556", "T1556.001", "T1556.002", "T1556.003", "T1556.004", "T1556.005",
    "T1556.006", "T1556.007", "T1556.008", "T1556.009",
    "T1557", "T1557.001", "T1557.002", "T1557.003", "T1557.004",
    "T1558", "T1558.001", "T1558.002", "T1558.003", "T1558.004", "T1558.005",
    "T1559", "T1559.001", "T1559.002", "T1559.003",
    "T1560", "T1560.001", "T1560.002", "T1560.003",
    "T1561", "T1561.001", "T1561.002",
    "T1562", "T1562.001", "T1562.002", "T1562.003", "T1562.004", "T1562.005",
    "T1562.006", "T1562.007", "T1562.008", "T1562.009", "T1562.010", "T1562.011",
    "T1562.012", "T1562.013",
    "T1563", "T1563.001", "T1563.002",
    "T1564", "T1564.001", "T1564.002", "T1564.003", "T1564.004", "T1564.005",
    "T1564.006", "T1564.007", "T1564.008", "T1564.009", "T1564.010", "T1564.011",
    "T1564.012", "T1564.013", "T1564.014",
    "T1565", "T1565.001", "T1565.002", "T1565.003",
    "T1567", "T1567.001", "T1567.002", "T1567.003", "T1567.004",
    "T1568", "T1568.001", "T1568.002", "T1568.003",
    "T1569", "T1569.001", "T1569.002", "T1569.003",
    "T1573", "T1573.001", "T1573.002",
    "T1574", "T1574.001", "T1574.002", "T1574.003", "T1574.004", "T1574.005",
    "T1574.006", "T1574.007", "T1574.008", "T1574.009", "T1574.010", "T1574.011",
    "T1574.012", "T1574.013", "T1574.014",
    "T1578", "T1578.001", "T1578.002", "T1578.003", "T1578.004", "T1578.005",
    "T1583", "T1583.001", "T1583.002", "T1583.003", "T1583.004", "T1583.005",
    "T1583.006", "T1583.007", "T1583.008",
    "T1584", "T1584.001", "T1584.002", "T1584.003", "T1584.004", "T1584.005",
    "T1584.006", "T1584.007", "T1584.008",
    "T1585", "T1585.001", "T1585.002", "T1585.003",
    "T1586", "T1586.001", "T1586.002", "T1586.003",
    "T1587.001", "T1587.002", "T1587.003", "T1587.004",
    "T1588.001", "T1588.002", "T1588.003", "T1588.004", "T1588.005", "T1588.006",
    "T1588.007",
    # --- Reconnaissance tactic (T1589–T1598) ---
    "T1589", "T1589.001", "T1589.002", "T1589.003",
    "T1590", "T1590.001", "T1590.002", "T1590.003", "T1590.004", "T1590.005",
    "T1590.006",
    "T1591", "T1591.001", "T1591.002", "T1591.003", "T1591.004",
    "T1592", "T1592.001", "T1592.002", "T1592.003", "T1592.004",
    "T1593", "T1593.001", "T1593.002", "T1593.003",
    "T1594",
    "T1595", "T1595.001", "T1595.002", "T1595.003",
    "T1596", "T1596.001", "T1596.002", "T1596.003", "T1596.004", "T1596.005",
    "T1597", "T1597.001", "T1597.002",
    "T1598", "T1598.001", "T1598.002", "T1598.003", "T1598.004",
    # --- Late-numbered techniques ---
    "T1599", "T1599.001",
    "T1600", "T1600.001", "T1600.002",
    "T1601", "T1601.001", "T1601.002",
    "T1602", "T1602.001", "T1602.002",
    "T1606.001", "T1606.002",
    "T1608", "T1608.001", "T1608.002", "T1608.003", "T1608.004", "T1608.005",
    "T1608.006",
    "T1609", "T1612", "T1614", "T1614.001",
    "T1615", "T1620", "T1621", "T1647", "T1648", "T1649", "T1650", "T1651",
    "T1652", "T1653", "T1654", "T1656", "T1657", "T1659", "T1665", "T1666",
    "T1667", "T1668", "T1669", "T1671", "T1672", "T1673", "T1674", "T1675",
    "T1677", "T1678", "T1679", "T1680", "T1681",
]

# Deduplicate while preserving order
seen = set()
_deduped = []
for t in TECHNIQUES:
    if t not in seen:
        seen.add(t)
        _deduped.append(t)
TECHNIQUES = _deduped


if __name__ == "__main__":
    LOG_DIR = REPO / "artifacts" / "evidence" / "arda_prevention" / "batch_payload_logs"
    LOG_DIR.mkdir(parents=True, exist_ok=True)
    RUN_ID = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    JSONL_PATH = LOG_DIR / f"phase1_full_v5_payload_logs_{RUN_ID}.jsonl"

    print(f"Phase 1 Full v5 — Expanded {len(TECHNIQUES)}-technique enforcement sweep")
    print(f"Techniques: {len(TECHNIQUES)}")
    print()

    # Step 1: Pre-write all payloads BEFORE enforcement
    print(f"[*] Pre-writing {len(TECHNIQUES)} payload binaries to /tmp...")
    for tid in TECHNIQUES:
        tool_name = f"arda_{tid.replace('.', '_')}"
        binary_path = f"/tmp/{tool_name}.bin"
        with open(binary_path, "w") as f:
            f.write(f"#!/bin/bash\necho {tid}\n")
        os.chmod(binary_path, 0o755)
    print(f"[+] All payloads written")

    # Step 2: Start Arda enforcement in BACKGROUND
    print(f"[*] Starting Arda enforcement (240s window, 30s delay)...")
    env = os.environ.copy()
    env["ARDA_ENFORCE_SECONDS"] = "240"
    env["ARDA_ENFORCE_DELAY_SECONDS"] = "30"
    env["ARDA_LSM_CONTAINER_NAME"] = "arda-lsm-loader"

    arda_proc = subprocess.Popen(
        ["bash", "scripts/arda_lsm_start.sh"],
        cwd=str(REPO),
        env=env,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    )
    print(f"[+] Arda starting (PID {arda_proc.pid})")

    # Step 3: Wait for enforcement to ACTIVATE (seeding ~5s + 30s delay + 10s buffer)
    print(f"[*] Waiting 65s for enforcement to fully activate...")
    for remaining in range(65, 0, -5):
        time.sleep(5)
        print(f"  ...{remaining-5}s remaining", flush=True)
    print(f"[+] Enforcement should now be active")

    # Step 4: Execute all payloads while enforcement is ACTIVE
    print(f"[*] Executing {len(TECHNIQUES)} payloads (enforcement is active)...")
    denied = 0
    start_time = time.time()

    for i, tid in enumerate(TECHNIQUES, 1):
        tool_name = f"arda_{tid.replace('.', '_')}"
        binary_path = f"/tmp/{tool_name}.bin"

        started_at = datetime.now(timezone.utc).isoformat()
        stdout_out = ""
        stderr_out = ""
        rc = 0
        denied_flag = False
        exception_str = ""

        try:
            pid = os.fork()
            if pid == 0:
                try:
                    os.execve(binary_path, [binary_path], os.environ.copy())
                except PermissionError:
                    os._exit(126)
                except Exception:
                    os._exit(1)
            else:
                _, status = os.waitpid(pid, 0)
                rc = os.waitstatus_to_exitcode(status)
                if rc == 126:
                    denied_flag = True
                    stderr_out = f"[Errno 1] Operation not permitted: '{binary_path}'"
                    exception_str = "PermissionError(1, 'Operation not permitted')"
        except PermissionError:
            rc = 126
            denied_flag = True
            stderr_out = f"[Errno 1] Operation not permitted: '{binary_path}'"
            exception_str = "PermissionError(1, 'Operation not permitted')"
        except Exception as e:
            rc = 1
            exception_str = repr(e)

        if denied_flag:
            denied += 1

        record = {
            "technique_id": tid,
            "binary_path": binary_path,
            "started_at": started_at,
            "rc": rc,
            "denied": denied_flag,
            "stdout": stdout_out,
            "stderr": stderr_out,
            "exception": exception_str,
        }

        with open(JSONL_PATH, "a") as f:
            f.write(json.dumps(record) + "\n")

        if i % 50 == 0:
            elapsed = time.time() - start_time
            print(f"  [{i}/{len(TECHNIQUES)}] denied={denied} elapsed={elapsed:.1f}s")

    print(f"\nK0 denials: {denied}/{len(TECHNIQUES)} ({100.0*denied/len(TECHNIQUES):.1f}%)")
    print(f"Log: {JSONL_PATH}")

