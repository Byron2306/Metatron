#!/usr/bin/env python3
"""
run_arda_prevention_full_v2.py
===============================
Phase 1 Full v2: Execute 516 techniques through Arda using Python one-liners.

Key difference from v1: payloads are executed via `python3 -c` (direct Python invocation,
no shell fork, no /bin/bash shebang needed). This bypasses the shell entirely during
enforcement window.

Each payload: python3 -c "import os; os.execve('/tmp/arda_T...', ['/tmp/arda_T...'], {})"
This triggers the bprm_check_security hook directly without needing /bin/bash.
"""
...
# All 516 techniques needing K0 coverage
...
def main() -> None:
    ...
    print(f"Arda Prevention Full v2 — Python one-liner payloads")
    print(f"=" * 70)
    print(f"Techniques: {len(TECHNIQUES_NEEDING_K0)}")
    print()
    ...
    # Step 2: Wait for enforcement
    print(f"[*] Waiting {args.delay_seconds}s for enforcement to activate...")
    time.sleep(args.delay_seconds + 2)

    # Step 3: Execute payloads via inline os.execve (direct kernel hook, no spawn)
    print(f"[*] Executing {len(TECHNIQUES_NEEDING_K0)} payloads via inline os.execve()...")
    denied = 0
    ...
    for i, tid in enumerate(TECHNIQUES_NEEDING_K0, 1):
        ...
        # Try to exec it directly via os.execve (triggers bprm_check_security hook in kernel)
        # This call will NEVER return if it succeeds; if denied, raises OSError
        try:
            # Use os.execve to trigger the hook
            # Since this is inside Python, we fork first then exec in child
            pid = os.fork()
            if pid == 0:
        ...
        if i % 50 == 0 or i == len(TECHNIQUES_NEEDING_K0):
            pct = 100.0 * denied / i
            print(f"    [{i:3d}/{len(TECHNIQUES_NEEDING_K0)}] {denied:3d} denied ({pct:5.1f}%)")

    print()
    print(f"[+] Execution complete: {denied}/{len(TECHNIQUES_NEEDING_K0)} EPERM denials")
    ...
    # Step 5: Cleanup
    print(f"[*] Cleaning up...")
    subprocess.run(["/usr/bin/docker", "rm", "-f", args.container], capture_output=True, timeout=30)

    print(f"[+] Phase 1 Full v2 complete")
