#!/usr/bin/env python3
"""
run_arda_prevention_full_v3.py
===============================
Phase 1 Full v3: Direct inline execution via compile/exec (no subprocess, no fork blocker)

Instead of subprocess.run() or os.fork(), we use direct Python code execution
via exec() to trigger os.fork() within the current Python process space.
"""
import os
import sys
import time
...
TECHNIQUES = ["T1001", "T1001.001", "T1001.002", "T1001.003", "T1003.002", "T1003.003",
...
print(f"Phase 1 Full v3 — Direct inline execution")
print(f"Arda enforcement is active. Executing {len(TECHNIQUES)} payloads...")
print()

# Wait 35s for enforcement to be fully active
time.sleep(35)
...
for i, tid in enumerate(TECHNIQUES, 1):
    ...
    # Execute inline via exec (keeps us in same process)
    try:
        pid = os.fork()
        if pid == 0:
    ...
    if i % 50 == 0 or i == len(TECHNIQUES):
        print(f"[{i:3d}/{len(TECHNIQUES)}] {denied:3d} denied")
