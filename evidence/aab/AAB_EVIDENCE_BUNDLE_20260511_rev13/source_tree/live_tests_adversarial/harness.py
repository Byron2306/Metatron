...
class AdversarialHarness:
    ...
    def run(self, profile: AgentProfile) -> EngagementRecord:
        ...
        for i, action in enumerate(actions):
            ...
            router_http_status: Optional[int] = None
            req_bytes: int = 0
            resp_bytes: int = 0
            action_started_at: float = time.time()
            action_completed_at: float = action_started_at
            _req_payload = {
                "ip": class_ip,
            ...
            req_bytes = len(json.dumps(_req_payload).encode())
            try:
                ...
                router_http_status = resp.status_code
                ...
                if resp.status_code == 200:
                    ...
                    router_reasons = list(body.get("reasons") or [])
            except Exception:
                action_completed_at = time.time()
            ...
            record.actions.append({
                ...
                # Per-action network telemetry
                "action_started_at": action_started_at,
                "action_completed_at": action_completed_at,
                "router_roundtrip_ms": round((action_completed_at - action_started_at) * 1000, 3),
                "router_http_status": router_http_status,
                "req_bytes": req_bytes,
