...
class WebAgentBridge:
    ...
    def service_action(self, svc_name: str, action: str) -> dict:
        try:
            # Use local execution broker to enforce governance on service control actions
            executor = lambda: (
                (True, f"service_{action}_attempted", {"output": subprocess.run(['sc', action, svc_name], capture_output=True, text=True, timeout=15).stdout.strip()} )
            )
            if hasattr(self, 'agent') and getattr(self.agent, 'local_execution_broker', None):
                ok, msg, meta = self.agent.local_execution_broker.execute_sensitive_action(
                    principal="local_ui",
                    capability="service_control",
                    target=str(svc_name),
                    parameters={"action": action, "service": svc_name},
                    decision_context={},
                    token=None,
                    risk_hint={"source": "local_ui"},
                    executor=lambda: (subprocess.run(['sc', action, svc_name], capture_output=True, text=True, timeout=15).returncode == 0,
                                      f"service_{action}_attempted",
                                      {"stdout": subprocess.run(['sc', action, svc_name], capture_output=True, text=True, timeout=15).stdout.strip()}),
                )
                if ok:
                    return {"success": True, "message": msg, "details": meta.get('execution')}
                return {"success": False, "error": msg, "details": meta}

            # Fallback if broker not available
            result = subprocess.run(['sc', action, svc_name], capture_output=True, text=True, timeout=15)
                ...
                ok, msg, meta = self.agent.local_execution_broker.execute_sensitive_action(
                    ...
                    executor=lambda: (
                        subprocess.run([
                            ...
                            f'name=SeraphAI-Block-{remote_ip}', 'dir=out', 'action=block',
                            f'remoteip={remote_ip}', 'protocol=any'
                        ], capture_output=True, text=True, timeout=10),
                        subprocess.run([
                            'netsh', 'advfirewall', 'firewall', 'add', 'rule',
                            f'name=SeraphAI-Block-{remote_ip}-In', 'dir=in', 'action=block',
                            f'remoteip={remote_ip}', 'protocol=any'
                        ], capture_output=True, text=True, timeout=10),
                        (True, f"ip_blocked", {"ip": remote_ip})
                    )[-1],
                )
                if ok:
                    return {"success": True, "message": msg, "details": meta.get('execution')}
        ...
        try:
            if hasattr(self, 'agent') and getattr(self.agent, 'local_execution_broker', None):
                ok, msg, meta = self.agent.local_execution_broker.execute_sensitive_action(
                    ...
                    token=None,
                    risk_hint={"source": "local_ui"},
                    executor=lambda: (self.agent.remediation.quarantine_file(Path(file_path)), f"quarantine_attempted", {"filepath": file_path})
                )
                if ok:
                    self.quarantine_items.append({"path": meta.get('execution'), "original": file_path, "status": "quarantined"})
                    self.add_event("INFO", f"Quarantined {file_path}")
                    return {"success": True, "quarantined_path": meta.get('execution')}
                return {"success": False, "error": msg, "details": meta}
                    ...
                    try:
                        ...
                        if round_num < 3:
                            if platform.system() == 'Windows':
                                ...
                                try:
                                    if hasattr(self, 'agent') and getattr(self.agent, 'local_execution_broker', None):
                                        # Broker the priority change as a process_control action
                                        def _exec_nice_idle():
                                            if c > 50 and cur != psutil.IDLE_PRIORITY_CLASS:
                                                proc.nice(psutil.IDLE_PRIORITY_CLASS)
                                                return True, 'set_idle_priority', {'pid': pid}
                                            elif cur not in (psutil.BELOW_NORMAL_PRIORITY_CLASS, psutil.IDLE_PRIORITY_CLASS):
                                                proc.nice(psutil.BELOW_NORMAL_PRIORITY_CLASS)
                                                return True, 'set_below_normal_priority', {'pid': pid}
                                            return False, 'no_change', {'pid': pid}

                                        ok, msg, meta = self.agent.local_execution_broker.execute_sensitive_action(
                                            principal='local_ui',
                                            capability='process_control',
                                            target=str(pid),
                                            parameters={'pid': pid, 'action': 'nice_adjust'},
                                            decision_context={},
                                            token=None,
                                            risk_hint={'source': 'auto_reduce_cpu'},
                                            executor=_exec_nice_idle,
                                        )
                                        if not ok:
                                            continue
                                        action = msg
                                ...
                                if hasattr(self, 'agent') and getattr(self.agent, 'local_execution_broker', None):
                                    def _exec_affinity():
                                        ...
                                        if n_cores > 1:
                                            ...
                                            return True, 'affinity_reduced', {'pid': pid, 'from': n_cores, 'to': len(new_aff)}
                                        return False, 'no_change', {'pid': pid}
                                    ...
                                    ok, msg, meta = self.agent.local_execution_broker.execute_sensitive_action(
                                        ...
                                        capability='process_control',
                                        target=str(pid),
                                        parameters={'pid': pid, 'action': 'cpu_affinity'},
                                        decision_context={},
                                        token=None,
                                        risk_hint={'source': 'auto_reduce_cpu'},
                                        executor=_exec_affinity,
                                    )
                                    if not ok:
                                        continue
                                    action = msg
                                ...
                                if self._ar_aggressive:
                                    if hasattr(self, 'agent') and getattr(self.agent, 'local_execution_broker', None):
                                        def _exec_suspend():
                                            ...
                                            return True, 'suspended_10s', {'pid': pid}

                                        ok, msg, meta = self.agent.local_execution_broker.execute_sensitive_action(
                                            principal='local_ui',
                                            capability='process_control',
                                            target=str(pid),
                                            parameters={'pid': pid, 'action': 'suspend'},
                                            decision_context={},
                                            token=None,
                                            risk_hint={'source': 'auto_reduce_cpu'},
                                            executor=_exec_suspend,
                                        )
                                        if not ok:
                                            continue
                                        action = msg
                            ...
                            try:
                                if self._ar_aggressive and c > 30:
                                    if hasattr(self, 'agent') and getattr(self.agent, 'local_execution_broker', None):
                                        def _exec_terminate():
                                            proc.terminate()
                                            return True, 'terminated', {'pid': pid}
                                ...
                                try:
                                    ...
                                    if hasattr(self, 'agent') and getattr(self.agent, 'local_execution_broker', None):
                                        ...
                                        ok, msg, meta = self.agent.local_execution_broker.execute_sensitive_action(
                                            ...
                                            decision_context={},
                                            token=None,
                                            risk_hint={'source': 'auto_reduce_memory'},
                                            executor=_exec_kill,
                                        )
                                        if not ok:
                                            continue
        ...
        try:
            ...
            if platform.system() == 'Windows':
                try:
                    if hasattr(self, 'agent') and getattr(self.agent, 'local_execution_broker', None):
                        def _exec_cleanmgr():
                            subprocess.run(['cleanmgr', '/d', 'C', '/sagerun:1'], capture_output=True, timeout=120)
                            return True, 'cleanmgr_triggered', {}

                        ok, msg, meta = self.agent.local_execution_broker.execute_sensitive_action(
                            principal='local_ui',
                            capability='run_tool',
                            target='cleanmgr',
                            parameters={'tool': 'cleanmgr'},
                            decision_context={},
                            token=None,
                            risk_hint={'source': 'auto_reduce_disk'},
                            executor=_exec_cleanmgr,
                        )
                        if ok:
