...
curl -sS -X POST "${CONTROLLER_URL}/phase/seed-compromise" \
  -H "Content-Type: application/json" \
  -d '{"session_id":"'"${SESSION_ID}"'","node_id":"node-a-sacrificial","agent_class":"persistent_recon","model":"qwen2.5:0.5b"}' \
...
curl -sS -X POST "${CONTROLLER_URL}/phase/isolate" \
  -H "Content-Type: application/json" \
  -d '{"session_id":"'"${SESSION_ID}"'","node_id":"node-a-sacrificial","method":"nftables"}' \
...

{
  ...
  echo "controller_state=$(curl -sS ${CONTROLLER_URL}/state)"
...
curl -sS -X POST "${CONTROLLER_URL}/quorum/vote" \
  -H "Content-Type: application/json" \
  -d '{"session_id":"'"${SESSION_ID}"'","node_id":"node-a-sacrificial","vote":"promote-self"}' \
...
curl -sS -X POST "${CONTROLLER_URL}/lorien/witness-recover" \
  -H "Content-Type: application/json" \
  -d '{"session_id":"'"${SESSION_ID}"'","target_node_id":"node-a-sacrificial","witness_node_id":"node-a-sacrificial"}' \
  >"${OUT_DIR}/08_bad_recovery_witness_rejected.json"

capture_quorum_heartbeat_metric "phase4_bad_witness_rejected"

log "Phase 5: lawful recovery and rejoin"
curl -sS -X POST "${CONTROLLER_URL}/lorien/recover" \
  -H "Content-Type: application/json" \
  -d '{"session_id":"'"${SESSION_ID}"'","target_node_id":"node-a-sacrificial","witness_node_id":"node-b-witness","proof_hash":"fresh-tree-of-truth","policy_hash":"fresh-tree-of-order"}' \
  >"${OUT_DIR}/09_lawful_recovery_accepted.json"
...
curl -sS "${CONTROLLER_URL}/mandos/ledger?limit=200" >"${OUT_DIR}/13_mandos_ledger.json"
...
assertions = {
    ...
    "quorum_returns_lawful": lawful.get("quorum_state") == "lawful",
    "recovery_recorded_in_ledger": any(
        e.get("aatr_id") == "LORIEN" and e.get("surface") == "/lorien/recover" and e.get("decision") == "recover"
